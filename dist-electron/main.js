...
function topBoolOrEmptySchema(it) {
  ...
  if (schema === false) {
    ...
    gen.return(names_1$a.default.data);
  } else {
...
function checkMissingProp$1({ gen, data, it: { opts } }, properties2, missing) {
  return (0, codegen_1$Q.or)(...properties2.map((prop) => (0, codegen_1$Q.and)(noPropertyInData$1(gen, data, prop, opts.ownProperties), (0, codegen_1$Q._)`${missing} = ${prop}`)));
}
...
function isOwnProperty$1(gen, data, property) {
  return (0, codegen_1$Q._)`${hasPropFunc$1(gen)}.call(${data}, ${property})`;
}
code$2.isOwnProperty = isOwnProperty$1;
function propertyInData$1(gen, data, property, ownProperties) {
  const cond = (0, codegen_1$Q._)`${data}${(0, codegen_1$Q.getProperty)(property)} !== undefined`;
  return ownProperties ? (0, codegen_1$Q._)`${cond} && ${isOwnProperty$1(gen, data, property)}` : cond;
}
code$2.propertyInData = propertyInData$1;
function noPropertyInData$1(gen, data, property, ownProperties) {
  const cond = (0, codegen_1$Q._)`${data}${(0, codegen_1$Q.getProperty)(property)} === undefined`;
  return ownProperties ? (0, codegen_1$Q.or)(cond, (0, codegen_1$Q.not)(isOwnProperty$1(gen, data, property))) : cond;
}
...
function callValidateCode$1({ schemaCode, data, it: { gen, topSchemaRef, schemaPath, errorPath }, it }, func, context, passSchema) {
  ...
  const valCxt = [
    [names_1$9.default.instancePath, (0, codegen_1$Q.strConcat)(names_1$9.default.instancePath, errorPath)],
...
const newRegExp$1 = (0, codegen_1$Q._)`new RegExp`;
function usePattern$1({ gen, it: { opts } }, pattern2) {
  ...
  return gen.scopeValue("pattern", {
    key: rx.toString(),
    ref: rx,
    code: (0, codegen_1$Q._)`${regExp.code === "new RegExp" ? newRegExp$1 : (0, util_2$3.useFunc)(gen, regExp)}(${pattern2}, ${u})`
  });
}
...
function funcKeywordCode(cxt, def2) {
  ...
  function assignValid(_await = def2.async ? (0, codegen_1$P._)`await ` : codegen_1$P.nil) {
    const passCxt = it.opts.passContext ? names_1$8.default.this : names_1$8.default.self;
    const passSchema = !("compile" in def2 && !$data || def2.schema === false);
    gen.assign(valid2, (0, codegen_1$P._)`${_await}${(0, code_1$j.callValidateCode)(cxt, validateRef, passCxt, passSchema)}`, def2.modifying);
  }
...
function addErrs(cxt, errs) {
  ...
  gen.if((0, codegen_1$P._)`Array.isArray(${errs})`, () => {
    gen.assign(names_1$8.default.vErrors, (0, codegen_1$P._)`${names_1$8.default.vErrors} === null ? ${errs} : ${names_1$8.default.vErrors}.concat(${errs})`).assign(names_1$8.default.errors, (0, codegen_1$P._)`${names_1$8.default.vErrors}.length`);
...
function validateFunction({ gen, validateName, schema, schemaEnv, opts }, body) {
  if (opts.code.es5) {
    gen.func(validateName, (0, codegen_1$N._)`${names_1$7.default.data}, ${names_1$7.default.valCxt}`, schemaEnv.$async, () => {
      gen.code((0, codegen_1$N._)`"use strict"; ${funcSourceUrl(schema, opts)}`);
      destructureValCxtES5(gen, opts);
      gen.code(body);
    });
  } else {
    gen.func(validateName, (0, codegen_1$N._)`${names_1$7.default.data}, ${destructureValCxt(opts)}`, schemaEnv.$async, () => gen.code(funcSourceUrl(schema, opts)).code(body));
  }
}
function destructureValCxt(opts) {
  return (0, codegen_1$N._)`{${names_1$7.default.instancePath}="", ${names_1$7.default.parentData}, ${names_1$7.default.parentDataProperty}, ${names_1$7.default.rootData}=${names_1$7.default.data}${opts.dynamicRef ? (0, codegen_1$N._)`, ${names_1$7.default.dynamicAnchors}={}` : codegen_1$N.nil}}={}`;
}
function destructureValCxtES5(gen, opts) {
  gen.if(names_1$7.default.valCxt, () => {
    gen.var(names_1$7.default.instancePath, (0, codegen_1$N._)`${names_1$7.default.valCxt}.${names_1$7.default.instancePath}`);
    gen.var(names_1$7.default.parentData, (0, codegen_1$N._)`${names_1$7.default.valCxt}.${names_1$7.default.parentData}`);
    gen.var(names_1$7.default.parentDataProperty, (0, codegen_1$N._)`${names_1$7.default.valCxt}.${names_1$7.default.parentDataProperty}`);
    gen.var(names_1$7.default.rootData, (0, codegen_1$N._)`${names_1$7.default.valCxt}.${names_1$7.default.rootData}`);
    if (opts.dynamicRef)
      gen.var(names_1$7.default.dynamicAnchors, (0, codegen_1$N._)`${names_1$7.default.valCxt}.${names_1$7.default.dynamicAnchors}`);
  }, () => {
    gen.var(names_1$7.default.instancePath, (0, codegen_1$N._)`""`);
    gen.var(names_1$7.default.parentData, (0, codegen_1$N._)`undefined`);
    gen.var(names_1$7.default.parentDataProperty, (0, codegen_1$N._)`undefined`);
    gen.var(names_1$7.default.rootData, names_1$7.default.data);
    if (opts.dynamicRef)
      gen.var(names_1$7.default.dynamicAnchors, (0, codegen_1$N._)`{}`);
  });
}
function topSchemaObjCode(it) {
  ...
  validateFunction(it, () => {
    if (opts.$comment && schema.$comment)
      commentKeyword(it);
    checkNoDefault(it);
    gen.let(names_1$7.default.vErrors, null);
    gen.let(names_1$7.default.errors, 0);
    if (opts.unevaluated)
      resetEvaluated(it);
    typeAndKeywords(it);
    returnResults(it);
...
function subSchemaObjCode(it, valid2) {
  ...
  updateContext(it);
  checkAsyncSchema(it);
  const errsCount = gen.const("_errs", names_1$7.default.errors);
  typeAndKeywords(it, errsCount);
  gen.var(valid2, (0, codegen_1$N._)`${errsCount} === ${names_1$7.default.errors}`);
}
...
function commentKeyword({ gen, schemaEnv, schema, errSchemaPath, opts }) {
  ...
  if (opts.$comment === true) {
    gen.code((0, codegen_1$N._)`${names_1$7.default.self}.logger.log(${msg})`);
  } else if (typeof opts.$comment == "function") {
    const schemaPath = (0, codegen_1$N.str)`${errSchemaPath}/$comment`;
    const rootName = gen.scopeValue("root", { ref: schemaEnv.root });
    gen.code((0, codegen_1$N._)`${names_1$7.default.self}.opts.$comment(${msg}, ${schemaPath}, ${rootName}.schema)`);
  }
}
function returnResults(it) {
  const { gen, schemaEnv, validateName, ValidationError, opts } = it;
  if (schemaEnv.$async) {
    gen.if((0, codegen_1$N._)`${names_1$7.default.errors} === 0`, () => gen.return(names_1$7.default.data), () => gen.throw((0, codegen_1$N._)`new ${ValidationError}(${names_1$7.default.vErrors})`));
  } else {
    gen.assign((0, codegen_1$N._)`${validateName}.errors`, names_1$7.default.vErrors);
    if (opts.unevaluated)
      assignEvaluated(it);
    gen.return((0, codegen_1$N._)`${names_1$7.default.errors} === 0`);
  }
}
...
function schemaKeywords(it, types2, typeErrors, errsCount) {
  ...
  function groupKeywords(group) {
    ...
    if (!allErrors)
      gen.if((0, codegen_1$N._)`${names_1$7.default.errors} === ${errsCount || 0}`);
  }
}
...
class KeywordCxt {
  constructor(it, def2, keyword2) {
    ...
    if ("code" in def2 ? def2.trackErrors : def2.errors !== false) {
      this.errsCount = it.gen.const("_errs", names_1$7.default.errors);
    }
  }
...
function getData($data, { dataLevel, dataNames, dataPathArr }) {
  ...
  let data;
  if ($data === "")
    return names_1$7.default.rootData;
  if ($data[0] === "/") {
    if (!JSON_POINTER.test($data))
      throw new Error(`Invalid JSON-pointer: ${$data}`);
    jsonPointer = $data;
    data = names_1$7.default.rootData;
  } else {
...
const def$R = {
  ...
  code(cxt) {
    ...
    const len = it.opts.unicode === false ? (0, codegen_1$I._)`${data}.length` : (0, codegen_1$I._)`${(0, util_1$I.useFunc)(cxt.gen, ucs2length_1$1.default)}(${data})`;
...
const def$Q = {
  ...
  code(cxt) {
    ...
    if ($data) {
      ...
      const regExpCode = regExp.code === "new RegExp" ? (0, codegen_1$H._)`new RegExp` : (0, util_1$H.useFunc)(gen, regExp);
      const valid2 = gen.let("valid");
    ...
    if (opts.strictRequired) {
      ...
      for (const requiredKey of schema) {
        if ((props === null || props === void 0 ? void 0 : props[requiredKey]) === void 0 && !definedProperties.has(requiredKey)) {
          ...
          (0, util_1$G.checkStrictMode)(it, msg, it.opts.strictRequired);
        }
      }
    }
...
const def$M = {
  ...
  code(cxt) {
    ...
    function loopN2(i, j) {
      const eql = (0, util_1$F.useFunc)(gen, equal_1$5.default);
...
const def$L = {
  ...
  code(cxt) {
    ...
    if ($data || schema && typeof schema == "object") {
      cxt.fail$data((0, codegen_1$C._)`!${(0, util_1$E.useFunc)(gen, equal_1$4.default)}(${data}, ${schemaCode})`);
    } else {
...
const def$K = {
  ...
  code(cxt) {
    ...
    let eql;
    const getEql = () => eql !== null && eql !== void 0 ? eql : eql = (0, util_1$D.useFunc)(gen, equal_1$3.default);
    let valid2;
    if (useLoop || $data) {
...
const def$J = {
  ...
  code(cxt) {
    ...
    if (!Array.isArray(items2)) {
      (0, util_1$C.checkStrictMode)(it, '"additionalItems" is ignored when "items" is not an array of schemas');
      return;
    }
...
function validateAdditionalItems$1(cxt, items2) {
  ...
  } else if (typeof schema == "object" && !(0, util_1$C.alwaysValidSchema)(it, schema)) {
...
function validateTuple$1(cxt, extraItems, schArr = cxt.schema) {
  ...
  schArr.forEach((sch, i) => {
    if ((0, util_1$B.alwaysValidSchema)(it, sch))
      return;
  ...
  function checkStrictTuple(sch) {
    ...
    if (opts.strictTuples && !fullTuple) {
      ...
      (0, util_1$B.checkStrictMode)(it, msg, opts.strictTuples);
    }
  }
}
...
const def$F = {
  ...
  code(cxt) {
    ...
    if (max === void 0 && min === 0) {
      (0, util_1$z.checkStrictMode)(it, `"minContains" == 0 without "maxContains": "contains" keyword ignored`);
      return;
    }
    if (max !== void 0 && min > max) {
      (0, util_1$z.checkStrictMode)(it, `"minContains" > "maxContains" is always invalid`);
      cxt.fail();
      return;
    }
    ...
    function validateItems(_valid, block) {
      gen.forRange("i", 0, len, (i) => {
        cxt.subschema({
          keyword: "contains",
          dataProp: i,
          dataPropType: util_1$z.Type.Num,
          compositeRule: true
        }, _valid);
        block();
      });
...
const def$D = {
  ...
  code(cxt) {
    ...
    it.props = true;
    if (opts.removeAdditional !== "all" && (0, util_1$x.alwaysValidSchema)(it, schema))
      return;
    ...
    function isAdditional(key) {
      let definedProp;
      if (props.length > 8) {
        const propsSchema = (0, util_1$x.schemaRefOrVal)(it, parentSchema.properties, "properties");
    ...
    function additionalPropertyCode(key) {
      ...
      if (typeof schema == "object" && !(0, util_1$x.alwaysValidSchema)(it, schema)) {
        const valid2 = gen.name("valid");
    ...
    function applyAdditionalSchema(key, valid2, errors2) {
      const subschema2 = {
        keyword: "additionalProperties",
        dataProp: key,
        dataPropType: util_1$x.Type.Str
      };
      if (errors2 === false) {
...
const def$C = {
  ...
  code(cxt) {
    ...
    if (it.opts.unevaluated && allProps.length && it.props !== true) {
      it.props = util_1$w.mergeEvaluated.props(gen, (0, util_1$w.toHash)(allProps), it.props);
    }
    const properties2 = allProps.filter((p) => !(0, util_1$w.alwaysValidSchema)(it, schema[p]));
...
const def$B = {
  ...
  code(cxt) {
    ...
    const alwaysValidPatterns = patterns.filter((p) => (0, util_1$v.alwaysValidSchema)(it, schema[p]));
    ...
    function checkMatchingProperties(pat) {
      for (const prop in checkProperties) {
        if (new RegExp(pat).test(prop)) {
          (0, util_1$v.checkStrictMode)(it, `property ${prop} matches pattern ${pat} (use allowMatchingProperties)`);
        }
      }
    }
...
const def$w = {
  ...
  code(cxt) {
    ...
    if (parentSchema.then === void 0 && parentSchema.else === void 0) {
      (0, util_1$r.checkStrictMode)(it, '"if" without "then" and "else" is ignored');
    }
...
function hasSchema$1(it, keyword2) {
  ...
  return schema !== void 0 && !(0, util_1$r.alwaysValidSchema)(it, schema);
}
_if$1.default = def$w;
var thenElse$1 = {};
...
const def$v = {
  ...
  code({ keyword: keyword2, parentSchema, it }) {
    if (parentSchema.if === void 0)
      (0, util_1$q.checkStrictMode)(it, `"${keyword2}" without "if" is ignored`);
  }
};
...
function requireApplicability() {
  ...
  function schemaHasRulesForType2({ schema, self: self2 }, type2) {
    const group = self2.RULES.types[type2];
...
const __dirname$1 = app$1.getAppPath();
const APP_ROOT = __dirname$1;
process.env.ELECTRON_DISABLE_SECURITY_WARNINGS = "true";
process.env.APP_ROOT = APP_ROOT;
const VITE_DEV_SERVER_URL = process.env["VITE_DEV_SERVER_URL"];
const MAIN_DIST = path$6.join(APP_ROOT, "dist-electron");
const RENDERER_DIST = path$6.join(APP_ROOT, "dist");
process.env.VITE_PUBLIC = VITE_DEV_SERVER_URL ? path$6.join(APP_ROOT, "public") : RENDERER_DIST;
let win;
function createWindow() {
  ...
  win = new BrowserWindow({
    ...
    webPreferences: {
      preload: path$6.join(MAIN_DIST, "preload.mjs")
    },
...
ipcMain$1.on("electron-store-get", async (event, val) => {
  event.returnValue = store.get(val);
});
ipcMain$1.on("electron-store-set", async (_event, key, val) => {
  store.set(key, val);
});
ipcMain$1.on("electron-store-delete", async (_event, key) => {
  store.delete(key);
});
...
ipcMain$1.on("reload-window", () => {
  win == null ? void 0 : win.webContents.reload();
});
ipcMain$1.on("force-reload-window", () => {
  win == null ? void 0 : win.webContents.reloadIgnoringCache();
});
ipcMain$1.on("toggle-devtools", () => {
  win == null ? void 0 : win.webContents.toggleDevTools();
});
ipcMain$1.on("zoom-in", () => {
  const currentZoom = (win == null ? void 0 : win.webContents.getZoomLevel()) || 0;
  win == null ? void 0 : win.webContents.setZoomLevel(currentZoom + 0.5);
});
ipcMain$1.on("zoom-out", () => {
  const currentZoom = (win == null ? void 0 : win.webContents.getZoomLevel()) || 0;
  win == null ? void 0 : win.webContents.setZoomLevel(currentZoom - 0.5);
});
ipcMain$1.on("zoom-reset", () => {
  win == null ? void 0 : win.webContents.setZoomLevel(0);
});
ipcMain$1.on("toggle-fullscreen", () => {
  if (win) {
    win.setFullScreen(!win.isFullScreen());
  }
});
ipcMain$1.on("print-window", (event) => {
  const window2 = BrowserWindow.fromWebContents(event.sender);
  if (window2) {
    window2.webContents.print({
      silent: false,
...
ipcMain$1.handle("preview-window", async (event, title2) => {
  const window2 = BrowserWindow.fromWebContents(event.sender);
  if (!window2) return false;
  try {
    const data = await window2.webContents.printToPDF({
      printBackground: true,
      margins: {
        top: 0,
        bottom: 0,
    ...
    const safeTitle = (title2 || "Print-Preview").replace(/[^a-z0-9\u0E00-\u0E7F\-\_\s]/gi, "").trim();
    const filename = `${safeTitle}.pdf`;
    const tempPath = path$6.join(os$1.tmpdir(), filename);
    fs$3.writeFileSync(tempPath, data);
    const previewWindow = new BrowserWindow({
      width: 1e3,
      height: 900,
      title: title2 || "Print Preview",
      icon: path$6.join(process.env.VITE_PUBLIC, "electron-vite.svg"),
      webPreferences: {
        plugins: true
        // Enable PDF viewer
    ...
    console.error("Failed to generate PDF preview:", error2);
    return false;
  }
});
