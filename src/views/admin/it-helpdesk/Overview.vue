...
import { Activity, ArrowDownRight, ArrowUpRight, CheckCircle2, ClipboardList, Clock, Star, Target, Ticket, User, Zap } from 'lucide-vue-next';
...
const ticketStats = computed(() => {
  if (!tickets.value.length) {
    return {
      ...
      open: 0, openTrend: 0,
      resolved: 0, resolvedTrend: 0,
      fcrPercent: '0%', fcrPercentTrend: 0,
      avgFcr: '0m', avgFcrTrend: 0,
      bestTime: '0m', bestTimeTrend: 0,
    };
  }
  ...
  const filterByDate = (ts: ITTicket[], start: Date, end?: Date) => ts.filter(t => {
  ...
  const currentOpen = currentMonthTickets.filter(t => t.status !== 'Resolved' && t.status !== 'Closed' && t.status !== 'Cancelled').length;
  const currentResolved = currentMonthTickets.filter(t => t.status === 'Resolved' || t.status === 'Closed');
  ...
  const currentAvgMs = currentResolved.length ? currentTotalMs / currentResolved.length : 0;
  if (currentMinMs === Infinity) currentMinMs = 0;
  const currentFcrPercent = currentResolved.length ? Math.round((currentFcrCount / currentResolved.length) * 100) : 0;


  // Last Month Data
  const lastMonthTickets = filterByDate(tickets.value, lastMonthStart, lastMonthEnd);
  const lastTotal = lastMonthTickets.length;
  const lastOpen = lastMonthTickets.filter(t => t.status !== 'Resolved' && t.status !== 'Closed' && t.status !== 'Cancelled').length;
  const lastResolved = lastMonthTickets.filter(t => t.status === 'Resolved' || t.status === 'Closed');
  ...
  const lastFcrPercent = lastResolved.length ? Math.round((lastFcrCount / lastResolved.length) * 100) : 0;
  ...
  return {
    total: currentTotal, totalTrend,
    open: currentOpen, openTrend,
    resolved: currentResolved.length, resolvedTrend,
    fcrPercent: currentFcrPercent + '%', fcrPercentTrend,
    avgFcr: formatDuration(currentAvgMs), avgFcrTrend,
    bestTime: formatDuration(currentMinMs), bestTimeTrend,
  };
});
...
const replyTimeStats = computed(() => {
  ...
  const hrs = (t: ITTicket) => (new Date(t.resolvedAt!).getTime() - new Date(t.createdAt).getTime()) / 3600000;
  ...
  return { lt1h, h1to8, h8to24, gt24h, noReply, total, resolvedCount: total - noReply };
});
...
const replyTimeSlices = computed(() => {
  ...
  const createSlice = (label: string, color: string, count: number) => ({ label, color, count });
  const data = [
  ...
  return data.map(slice => {
    ...
    const percent = grandTotal > 0 ? Math.round((slice.count / grandTotal) * 100) : 0;

    const isChartSlice = slice.label !== 'No Replies';

    // Only calculate path if it's a resolved ticket slice and has count > 0
    let path = undefined, lx = undefined, ly = undefined;
...
const categoryStats = computed(() => {
  ...
  tickets.value.forEach((t: ITTicket) => { if (t.category) counts[t.category] = (counts[t.category] || 0) + 1; });
  const total = tickets.value.length || 1;
...
const agentStats = computed(() => {
  ...
  tickets.value.filter((t: ITTicket) => t.status === 'Resolved' || t.status === 'Closed')
    .forEach((t: ITTicket) => {
      if (t.assignee) {
        const id = t.assigneeId || t.assignee.id;
        if (!counts[id]) counts[id] = { name: t.assignee.displayName || 'Unknown Agent', count: 0 };
        counts[id].count++;
      }
    });
...
const recentTicketsList = computed(() =>
  [...tickets.value].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).slice(0, 5)
);

const categoryColors = ['#0D9488', '#3B82F6', '#F59E0B', '#8B5CF6', '#F43F5E', '#FB923C'];
  ...
  <div class="min-h-full space-y-6 animate-in fade-in duration-700">
    ...
    <!-- ── Stats Row ───────────────────────────────────────      <!-- Top 6 Cards -->
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-6 mb-8">
        ...
        <Card class="hover:shadow-lg transition-shadow duration-300 border-none shadow-sm bg-white">
          <CardHeader class="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle class="text-sm font-medium text-muted-foreground">Created Tickets</CardTitle>
            <Ticket class="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div class="flex items-baseline space-x-2">
              <div class="text-3xl font-bold text-gray-900">{{ ticketStats.total.toLocaleString() }}</div>
              <div class="flex items-center text-xs font-medium" :class="ticketStats.totalTrend >= 0 ? 'text-green-500' : 'text-red-500'">
                {{ ticketStats.totalTrend >= 0 ? '+' : '' }}{{ ticketStats.totalTrend }}%
                <ArrowUpRight v-if="ticketStats.totalTrend >= 0" class="h-3 w-3 ml-0.5" />
                <ArrowDownRight v-else class="h-3 w-3 ml-0.5" />
              </div>
            </div>
            <p class="text-xs text-muted-foreground mt-1">Compare to last month</p>
          </CardContent>
        </Card>
        ...
        <Card class="hover:shadow-lg transition-shadow duration-300 border-none shadow-sm bg-white">
          <CardHeader class="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle class="text-sm font-medium text-muted-foreground">Unsolved Tickets</CardTitle>
            <Clock class="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div class="flex items-baseline space-x-2">
              <div class="text-3xl font-bold text-gray-900">{{ ticketStats.open.toLocaleString() }}</div>
              <div class="flex items-center text-xs font-medium" :class="ticketStats.openTrend <= 0 ? 'text-green-500' : 'text-red-500'">
                 ...
                 -->
                {{ ticketStats.openTrend >= 0 ? '+' : '' }}{{ ticketStats.openTrend }}%
                <ArrowUpRight v-if="ticketStats.openTrend >= 0" class="h-3 w-3 ml-0.5" />
                <ArrowDownRight v-else class="h-3 w-3 ml-0.5" />
              </div>
            </div>
            <p class="text-xs text-muted-foreground mt-1">Compare to last month</p>
        ...
        <Card class="hover:shadow-lg transition-shadow duration-300 border-none shadow-sm bg-white">
          <CardHeader class="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle class="text-sm font-medium text-muted-foreground">Solved Tickets</CardTitle>
            <CheckCircle2 class="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
             <div class="flex items-baseline space-x-2">
              <div class="text-3xl font-bold text-gray-900">{{ ticketStats.resolved.toLocaleString() }}</div>
              <div class="flex items-center text-xs font-medium" :class="ticketStats.resolvedTrend >= 0 ? 'text-green-500' : 'text-red-500'">
                {{ ticketStats.resolvedTrend >= 0 ? '+' : '' }}{{ ticketStats.resolvedTrend }}%
                <ArrowUpRight v-if="ticketStats.resolvedTrend >= 0" class="h-3 w-3 ml-0.5" />
                <ArrowDownRight v-else class="h-3 w-3 ml-0.5" />
              </div>
            </div>
            <p class="text-xs text-muted-foreground mt-1">Compare to last month</p>
        ...
        <Card class="hover:shadow-lg transition-shadow duration-300 border-none shadow-sm bg-white">
          <CardHeader class="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle class="text-sm font-medium text-muted-foreground">Best Resolution Time</CardTitle>
            <Zap class="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
             <div class="flex items-baseline space-x-2">
              <div class="text-3xl font-bold text-gray-900">{{ ticketStats.bestTime }}</div>
              <div class="flex items-center text-xs font-medium" :class="ticketStats.bestTimeTrend <= 0 ? 'text-green-500' : 'text-red-500'">
                {{ ticketStats.bestTimeTrend >= 0 ? '+' : '' }}{{ ticketStats.bestTimeTrend }}%
                <ArrowUpRight v-if="ticketStats.bestTimeTrend >= 0" class="h-3 w-3 ml-0.5" />
                <ArrowDownRight v-else class="h-3 w-3 ml-0.5" />
              </div>
            </div>
            <p class="text-xs text-muted-foreground mt-1">Compare to last month</p>
        ...
        <Card class="hover:shadow-lg transition-shadow duration-300 border-none shadow-sm bg-white">
          <CardHeader class="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle class="text-sm font-medium text-muted-foreground">First Call Response Rate</CardTitle>
            <Target class="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
             <div class="flex items-baseline space-x-2">
              <div class="text-3xl font-bold text-gray-900">{{ ticketStats.fcrPercent }}</div>
              <div class="flex items-center text-xs font-medium" :class="ticketStats.fcrPercentTrend >= 0 ? 'text-green-500' : 'text-red-500'">
                {{ ticketStats.fcrPercentTrend >= 0 ? '+' : '' }}{{ ticketStats.fcrPercentTrend }}%
                <ArrowUpRight v-if="ticketStats.fcrPercentTrend >= 0" class="h-3 w-3 ml-0.5" />
                <ArrowDownRight v-else class="h-3 w-3 ml-0.5" />
              </div>
            </div>
            <p class="text-xs text-muted-foreground mt-1">Compare to last month</p>
        ...
        <Card class="hover:shadow-lg transition-shadow duration-300 border-none shadow-sm bg-white">
          <CardHeader class="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle class="text-sm font-medium text-muted-foreground">Average First Call Response</CardTitle>
            <Activity class="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
             <div class="flex items-baseline space-x-2">
              <div class="text-3xl font-bold text-gray-900">{{ ticketStats.avgFcr }}</div>
              <div class="flex items-center text-xs font-medium" :class="ticketStats.avgFcrTrend <= 0 ? 'text-green-500' : 'text-red-500'">
                 <!-- For time, lower is usually better. So < 0 is Green. -->
                {{ ticketStats.avgFcrTrend >= 0 ? '+' : '' }}{{ ticketStats.avgFcrTrend }}%
                <ArrowUpRight v-if="ticketStats.avgFcrTrend >= 0" class="h-3 w-3 ml-0.5" />
                <ArrowDownRight v-else class="h-3 w-3 ml-0.5" />
              </div>
            </div>
            <p class="text-xs text-muted-foreground mt-1">Compare to last month</p>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <Card class="border-none shadow-2xl shadow-black/5 rounded-[10px] overflow-hidden">
        ...
        <CardContent class="px-6 pb-5">
           <div class="space-y-3 max-h-[220px] overflow-y-auto pr-2 scrollbar-hide">
      ...
      <Card class="border-none shadow-2xl shadow-black/5 rounded-[10px] overflow-hidden">
        <CardHeader class="px-6 pt-5 pb-3">
          <div class="flex items-center justify-between gap-2">
            ...
            <span class="text-lg font-black text-primary">{{ ticketStats.fcrPercent }}</span>
          </div>
        </CardHeader>
        <CardContent class="px-6 pb-5">
          <div class="relative w-full aspect-[2/1] max-w-[240px] mx-auto mt-4 mb-2">
             <!-- Gauge Background -->
             <svg viewBox="0 0 200 100" class="w-full h-full text-center">
                <!-- Segments: Red, Orange, Yellow, Light Green, Green -->
                <!-- Red (180deg -> 144deg) -->
                <path d="M 30 100 A 70 70 0 0 1 43.37 58.85" fill="none" stroke="#ef4444" stroke-width="35" stroke-linecap="butt" />
                <!-- Orange (144deg -> 108deg) -->
                <path d="M 43.37 58.85 A 70 70 0 0 1 78.37 33.43" fill="none" stroke="#f97316" stroke-width="35" stroke-linecap="butt" />
                <!-- Yellow (108deg -> 72deg) -->
                <path d="M 78.37 33.43 A 70 70 0 0 1 121.63 33.43" fill="none" stroke="#eab308" stroke-width="35" stroke-linecap="butt" />
                <!-- Light Green (72deg -> 36deg) -->
                <path d="M 121.63 33.43 A 70 70 0 0 1 156.63 58.85" fill="none" stroke="#84cc16" stroke-width="35" stroke-linecap="butt" />
                <!-- Green (36deg -> 0deg) -->
                <path d="M 156.63 58.85 A 70 70 0 0 1 170 100" fill="none" stroke="#22c55e" stroke-width="35" stroke-linecap="butt" />

                <!-- Emojis inside segments (Centered at R=70) -->
                <text x="33" y="82" font-size="16" text-anchor="middle" dominant-baseline="middle">😠</text>
                <text x="58" y="48" font-size="16" text-anchor="middle" dominant-baseline="middle">🙁</text>
                <text x="100" y="35" font-size="16" text-anchor="middle" dominant-baseline="middle">😐</text>
                <text x="142" y="48" font-size="16" text-anchor="middle" dominant-baseline="middle">🙂</text>
                <text x="167" y="82" font-size="16" text-anchor="middle" dominant-baseline="middle">😁</text>
               ...
               <g :style="{ 
                 ...
                 <!-- Thinner Needle Base -->
                 <polygon points="100,105 92,100 100,30 108,100" fill="#1f2937" />
                 <circle cx="100" cy="100" r="6" fill="#1f2937" />
                 <circle cx="100" cy="100" r="2" fill="#fff" />
               </g>
             </svg>
          </div>
          <div class="text-center text-sm font-black text-muted-foreground/60 mt-3 uppercase tracking-tight">
          ...
          <div class="flex items-center gap-8 justify-center">
            <div class="relative shrink-0 w-52 h-52">
              <svg viewBox="0 0 100 100" class="w-full h-full -rotate-90 transform">
                <path v-for="(slice, i) in replyTimeSlices" :key="i"
              ...
              <div class="absolute inset-0 pointer-events-none">
                <div v-for="(slice, i) in replyTimeSlices" :key="i"
              ...
              <!-- Center Text -->
              <div class="absolute inset-8 rounded-full bg-card shadow-inner flex flex-col items-center justify-center pointer-events-none">
                <span class="text-4xl font-black tracking-tighter text-foreground">{{ replyTimeStats.resolvedCount }}</span>
                <span class="text-xs font-bold uppercase tracking-widest text-muted-foreground/60">Tickets</span>
              </div>
            </div>

            <!-- Legend -->
            <div class="flex-1 space-y-3 min-w-[120px]">
              <div v-for="(item, i) in replyTimeSlices" :key="i" class="flex items-center gap-2">
                <span class="w-3 h-3 rounded-sm shrink-0" :style="{ backgroundColor: item.color }"></span>
                <span class="flex-1 text-sm font-bold text-foreground/80">{{ item.label }}</span>
                <span class="text-sm font-black text-foreground">{{ item.count }}</span>
                <span class="text-[0.65rem] font-bold text-muted-foreground/60 w-9 text-right">
                  {{ item.percent }}%
                </span>
              </div>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-10 gap-6">
      ...
      <!-- Recent Tickets (70%) -->
      <Card class="col-span-1 lg:col-span-7 border-none shadow-2xl shadow-black/5 rounded-[10px] overflow-hidden">
        ...
        <CardContent class="px-6 pb-5">
          <div v-if="!recentTicketsList.length" class="flex flex-col items-center gap-3 py-12 opacity-20">
            <Activity class="w-10 h-10" />
            <p class="text-xs font-black uppercase tracking-widest">No Tickets</p>
          </div>
          <div v-else class="space-y-1">
            <!-- Table Header -->
            <div class="grid grid-cols-[4px_1fr_90px_110px_70px_100px] gap-4 px-2 py-2 text-[0.6rem] font-black uppercase tracking-widest text-muted-foreground/50 border-b border-border/30">
              <div></div>
            ...
            <div v-for="ticket in recentTicketsList" :key="ticket.id"
              ...
              <div class="shrink-0 w-1 h-10 rounded-full"
                :class="getPriorityColor(ticket.priority)"
              ></div>

              <!-- Content -->
              <div class="min-w-0">
                <p class="text-sm font-bold text-foreground truncate group-hover:text-primary transition-colors leading-tight">{{ ticket.title }}</p>
              ...
              <div class="flex justify-center">
                <span class="inline-flex items-center px-2 py-0.5 rounded-[6px] text-[0.65rem] font-bold bg-slate-100 text-slate-700 border border-slate-200 shadow-sm">
                  {{ getDuration(ticket) }}
                </span>
              </div>
              <!-- Opened Time -->
              <div class="text-[0.65rem] font-bold text-muted-foreground tabular-nums text-center">
              ...
              <div class="flex items-center justify-center gap-0.5">
                <Star v-for="n in 5" :key="n" 
                  ...
                  :class="n <= (ticket.rating || 0) ? 'fill-amber-400 text-amber-400' : 'text-slate-200 fill-slate-50'" 
                />
              </div>
              <!-- Status Badge -->
              <div class="flex justify-center">
      ...
      <Card class="col-span-1 lg:col-span-3 border-none shadow-2xl shadow-black/5 rounded-[10px] overflow-hidden">
        <CardHeader class="px-6 pt-5 pb-3">
          <CardTitle class="text-base font-black tracking-tight flex items-center gap-2">
