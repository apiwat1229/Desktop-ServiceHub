...
const { machines, deleteMachine, getMachineStats } = useMaintenance();
...
const handleDeleteMachine = async (id: string) => {
  if (await confirm(t('services.maintenance.messages.confirmDeleteMachine'))) {
    deleteMachine(id);
    toast.success(t('services.maintenance.messages.machineRemoved'));
  }
};
...
const columns = computed<ColumnDef<any>[]>(() => [
  {
    accessorKey: 'name',
    header: t('services.maintenance.machines.columns.machineModel'),
    cell: ({ row }) => {
      ...
      return h('div', { class: 'flex items-center gap-3 group/name' }, [
        ...
        h('div', { class: 'min-w-0 flex-1' }, [
          ...
          h(
            ...
            machine.location || t('services.maintenance.notAssigned')
          ),
        ]),
      ]);
    },
    ...
    cell: ({ row }) => {
      ...
      return h('div', { class: 'flex flex-col gap-1' }, [
        h(
          ...
          () => t(`services.maintenance.forms.machine.status${status}`)
        ),
        h(
          'span',
          { class: 'text-[9px] text-slate-400 font-medium' },
          `${stats.count} ${t('services.maintenance.entriesRecorded')}`
        ),
      ]);
    },
  },
  {
    ...
    header: t('services.maintenance.machines.columns.cost'),
    cell: ({ row }) => {
      const stats = getMachineStats(row.original.id);
      ...
      <div class="flex items-center justify-between gap-4">
        <div class="space-y-1">
          ...
          <p class="text-sm text-slate-500 font-medium">
            {{ t('services.maintenance.machines.subtitle') }}
          </p>
        </div>
